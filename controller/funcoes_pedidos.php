...
function listaPedidos($conexao, $idempresa) {
	$pedidos = array();
	$resultado = mysqli_query($conexao, "select idpedido,nomecliente,nomeprato,quantidade,observacao,cidade,bairro,rua,pedidos.numero,complemento,descrpagamento,status from pedidos,cardapios,forma_pagamento,empresa where pedidos.idempresa = empresa.idempresa  and pedidos.idcardapio = cardapios.idcardapio and pedidos.idformapagamento = forma_pagamento.idformapagamento and pedidos.idempresa = '{$idempresa}'");
